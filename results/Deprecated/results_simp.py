...
import csv
import re
from sympy import simplify, expand, refine, Q,cse, sqrt
...
def bingosimplify(expr, x_type='X_0', round_num=1):
    if isinstance(expr, str):
        ...
        for i in range(len(v_filtered)):
            if i <= 25:
                variables.append(string.ascii_uppercase[count])
            else:
                variables.append(string.ascii_uppercase[count] + string.ascii_lowercase[n])
            count += 1
            if count >= 26:
                count = 0
                n += 1
...
def expr_parse_expand(round_num, v_filtered,
    ...
    for i in range(0, len(v_filtered)):
        ...
        print('\n',v_filtered,'---------',len(v_filtered),variables,len(variables),'\n')
        replacements2 = [(variables[j], v_filtered[j])
                         for j in range(len(v_filtered))]  # Logs what replacements were made, so they can be reinserted later.
...
def Recent_beam(parent_dir):
    ...
    for i in Trial_Vec:
        ...
        best_ind = archipelago.get_best_individual()
        print(best_ind)
        F_X_0 = bingosimplify(str(best_ind))
        # write the fitness data to a new row
        with open('/uufs/chpc.utah.edu/common/home/u1008557/trials_gen2/Fitness_data.csv', 'a') as csvfile:
